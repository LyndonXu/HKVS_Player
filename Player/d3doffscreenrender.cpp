...
void D3DOffscreenRender::ChangehWnd(HWND hWnd)
{
	m_hWnd = hWnd;
}
...
int D3DOffscreenRender::init(int index, HWND hWnd, int width, int height)
{
	...
	__try
	{
		...
		PRINT("D3DOffscreenRender::init，first initDirect3D D3DFMT_A2R10G10B10!\n");
		iret = initDirect3D(m_d3dformat);
		if (iret < 0)
		{
			PRINT("D3DOffscreenRender::init,first initDirect3D D3DFMT_A2R10G10B10 failed!\n");
			destroyDirect3D();
			return -1;
		}
		else
...
int D3DOffscreenRender::initDirect3D(D3DFORMAT _format)
{
	...
	{
		PRINT("initDirect3D D3DFMT_A2R10G10B10!\n");
	}
	else
	{
		PRINT("initDirect3D D3DFMT_YV12!\n");
	}
	...
	if (FAILED(lRet))
	{
		PRINT("initDirect3D CreateDevice failed!left = %d,right = %d,top = %d,bottom = %d\n",
	...
	if (FAILED(lRet))
	{
		PRINT("initDirect3D GetDeviceCaps failed!\n");
		return -1;
	}
	...
	if (FAILED(lRet))
	{
		PRINT("initDirect3D CreateOffscreenPlainSurface failed!\n");
		return -1;
	}
	...
	{
		PRINT("initDirect3D D3DFMT_YV12! success\n");
	}
	...
	int width, int height, int BytePerSample)
{
	...
	if (m_BytePerSample <= 1 && m_d3dformat != m_8bit_d3dformat)
	{
		...
		if (m_d3dformat == D3DFMT_A8R8G8B8)
		{
			PRINT("m_BytePerSample <= 1 ，initDirect3D D3DFMT_A8R8G8B8!\n");
		}
		else
		{
			PRINT("m_BytePerSample <= 1 ，initDirect3D D3DFMT_YV12!\n");
		}
		int iret = initDirect3D(m_d3dformat);
		if (iret < 0)
		{
			...
			{
				PRINT("D3DOffscreenRender::render，initDirect3D D3DFMT_YV12! failed\n");
			}
			return -1;
		}
	}
	...
	else
	{
		if (m_d3dformat == D3DFMT_A2R10G10B10)
		{
			...
			{
				m_pDirect3DSurfaceRender->UnlockRect();
				PRINT("D3DOffscreenRender::process_10bit_rgbRender failed!\n");
				return -1;
			}
		}
		else
...
void D3DOffscreenRender::check_stretch()
{
	...
	m_yuv2rgb10 = yuv2rgb10_sse2_stretch;
	m_yuv2rgb8 = yuv2rgb8_sse2_2;
